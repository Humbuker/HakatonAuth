...
public class sqlManager {
    ...
    private String username = "studio2";
    private String password = "kasgdkjahgdja";
    private String jdbc_connect_line = "jdbc:mysql://185.143.238.18/project";
        ...
        new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... params) {
                try {
                    ...
                    connection = DriverManager.getConnection(jdbc_connect_line, username, password);
                    statement = connection.createStatement();
                    if (!statement.executeQuery("SELECT * FROM `users` WHERE `email` = '" + email.toLowerCase().trim() + "'").isBeforeFirst()) {
                        resultBoolean = false;
                        return null;
                    }
                    resultBoolean = true;
...
try {
                    new AsyncTask<Void, Void, Void>() {
                        ...
                        @Override protected Void doInBackground(Void... params){
                            ...
                            try{
                                Class.forName("com.mysql.jdbc.Driver");
                                conn = DriverManager.getConnection("jdbc:mysql://185.143.238.18:3306/project", "studio2", "kasgdkjahgdja");
                                statement = conn.createStatement();
                                System.out.println("Connected.");
                                if(loginPlace.getText().toString().length() < 10 || passwordPlace.getText().toString().length() < 8){
                                    Toast.makeText(getApplicationContext(), "Sended data is short.", Toast.LENGTH_LONG).show();
                                    return null;
                                }
                                if(isUserRegistred(loginPlace.getText().toString().toString().trim()) == false){
                                    Toast.makeText(getApplicationContext(), "No in database.", Toast.LENGTH_LONG).show();
                                    return null;
                                }
                            }catch (Exception ev){
                        ...
                        public boolean isUserRegistred(String email){
                            try {
                                if(!statement.executeQuery("SELECT * FROM `users` WHERE `email` = '"+ email.toLowerCase().trim() +"'").isBeforeFirst()) return false;
                                return true;
                            }catch (Exception e){
                                System.out.println(e.getMessage());
                                return false;
...
public class sqlManager extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        String query = "CREATE TABLE users (id INTEGER PRIMARY KEY, login TEXT, email TEXT, pass TEXT)";
        db.execSQL(query);
    }
    ...
    public boolean getUser(String login, String pass) {
        ...
        Cursor cursor = db.rawQuery("SELECT * FROM users WHERE login=? AND pass=?", selectionArgs);
